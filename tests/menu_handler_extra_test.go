...
func TestMenuGet_Success(t *testing.T) {
	...
	restRepo := new(MockRestaurantRepo)
	h := handlers.NewMenuHandler(menuRepo, restRepo)
	menu := &models.Menu{ID: "menu-1", RestaurantID: "rest-1", Name: "Lunch", Description: "desc"}
...
func TestMenuUpdate_NotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPut, "/menus/menu-404", bytes.NewBufferString(`{"name":"Updated"}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
