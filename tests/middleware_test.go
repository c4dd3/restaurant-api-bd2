...
func TestAuth_MissingHeader(t *testing.T) {
	...
	r := gin.New()
	r.GET("/protected", middleware.Auth(jwtSvc), func(c *gin.Context) { c.JSON(200, gin.H{"ok": true}) })
...
func TestAuth_ValidToken(t *testing.T) {
	gin.SetMode(gin.TestMode)
	jwtSvc := auth.NewJWTService()
	token, err := jwtSvc.GenerateToken(&models.User{ID: "user-1", Email: "user@test.com", Role: models.RoleClient})
	assert.NoError(t, err)
	r := gin.New()
	r.GET("/protected", middleware.Auth(jwtSvc), func(c *gin.Context) {
		claims := middleware.ExtractClaims(c)
		if claims == nil { c.JSON(500, gin.H{"error": "claims missing"}); return }
		c.JSON(200, gin.H{"user_id": claims.UserID, "role": claims.Role})
	})
	req, _ := http.NewRequest(http.MethodGet, "/protected", nil)
...
func TestAdminOnly_NoClaims(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.GET("/admin", middleware.AdminOnly(), func(c *gin.Context) { c.JSON(200, gin.H{"ok": true}) })
...
func TestAdminOnly_ClientForbidden(t *testing.T) {
	...
	r.GET("/admin", func(c *gin.Context) {
		c.Set(middleware.ClaimsKey, &models.Claims{UserID: "user-1", Role: models.RoleClient})
...
func TestAdminOnly_AdminAllowed(t *testing.T) {
	...
	r := gin.New()
	r.GET("/admin", func(c *gin.Context) {
		c.Set(middleware.ClaimsKey, &models.Claims{UserID: "admin-1", Role: models.RoleAdmin})
		middleware.AdminOnly()(c)
	}, func(c *gin.Context) { c.JSON(200, gin.H{"ok": true}) })
	req, _ := http.NewRequest(http.MethodGet, "/admin", nil)
	w := httptest.NewRecorder()
...
func TestExtractClaims_WrongType(t *testing.T) {
	...
	assert.Nil(t, middleware.ExtractClaims(c))
}
