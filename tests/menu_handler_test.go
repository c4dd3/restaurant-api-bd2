...
func TestMenuCreate_RestaurantNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/menus",
		bytes.NewBufferString(`{"restaurant_id":"rest-404","name":"Dinner","description":"desc","items":[{"name":"Pasta","description":"ok","price":12.5,"available":true}]}`))
...
func TestMenuCreate_Success(t *testing.T) {
	...
	r.POST("/menus", h.Create)
	req, _ := http.NewRequest(http.MethodPost, "/menus",
		bytes.NewBufferString(`{"restaurant_id":"rest-1","name":"Dinner","description":"desc","items":[{"name":"Pasta","description":"ok","price":12.5,"available":true}]}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
...
func TestMenuUpdate_Success(t *testing.T) {
	...
	restRepo := new(MockRestaurantRepo)
	h := handlers.NewMenuHandler(menuRepo, restRepo)
	updated := &models.Menu{ID: "menu-1", RestaurantID: "rest-1", Name: "Updated Menu", Description: "new desc"}
