...
func TestReservationCreate_RestaurantNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/reservations",
		bytes.NewBufferString(`{"restaurant_id":"rest-404","date":"2026-03-20T18:00:00Z","party_size":4,"notes":"window"}`))
...
func TestReservationCreate_InsufficientCapacity(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/reservations",
		bytes.NewBufferString(`{"restaurant_id":"rest-1","date":"2026-03-20T18:00:00Z","party_size":6,"notes":"window"}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
...
func TestReservationCreate_Success(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/reservations",
		bytes.NewBufferString(`{"restaurant_id":"rest-1","date":"2026-03-20T18:00:00Z","party_size":4,"notes":"window"}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
...
func TestReservationCancel_Forbidden(t *testing.T) {
	...
	restRepo := new(MockRestaurantRepo)
	h := handlers.NewReservationHandler(resRepo, restRepo)
	reservation := &models.Reservation{ID: "res-1", RestaurantID: "rest-1", UserID: "owner-user", Status: models.StatusPending}
