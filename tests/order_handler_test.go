...
func TestOrderCreate_RestaurantNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/orders",
		bytes.NewBufferString(`{"restaurant_id":"rest-404","items":[{"menu_item_id":"item-1","quantity":2}],"pickup":true}`))
...
func TestOrderCreate_MenuItemNotFound(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/orders",
		bytes.NewBufferString(`{"restaurant_id":"rest-1","items":[{"menu_item_id":"item-404","quantity":2}],"pickup":true}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
	assert.Equal(t, http.StatusNotFound, w.Code)
...
func TestOrderCreate_Success(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/orders",
		bytes.NewBufferString(`{"restaurant_id":"rest-1","items":[{"menu_item_id":"item-1","quantity":2},{"menu_item_id":"item-2","quantity":3}],"pickup":true}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
...
func TestOrderGet_Forbidden(t *testing.T) {
	...
	restRepo := new(MockRestaurantRepo)
	h := handlers.NewOrderHandler(orderRepo, menuRepo, restRepo)
	order := &models.Order{ID: "order-1", UserID: "owner-user", RestaurantID: "rest-1", Status: models.StatusPending}
