...
func TestRestaurantRepository_Create_And_FindByID(t *testing.T) {
	...
	rest := &models.Restaurant{
		...
		Address: "123 Test Street", Phone: "2222-3333",
		Description: "test restaurant", AdminID: admin.ID, Capacity: 80,
	}
...
func TestRestaurantRepository_FindAll(t *testing.T) {
	...
	repo := repository.NewRestaurantRepository(testDB)
	admin := createRepoTestUser(t, models.RoleAdmin)
	rest1 := &models.Restaurant{Name: fmt.Sprintf("FindAll A %d", time.Now().UnixNano()), Address: "A", Phone: "1111", AdminID: admin.ID, Capacity: 50}
	rest2 := &models.Restaurant{Name: fmt.Sprintf("FindAll B %d", time.Now().UnixNano()), Address: "B", Phone: "2222", AdminID: admin.ID, Capacity: 90}
	...
	for _, r := range restaurants {
		if r.ID == rest1.ID { found1 = true }
		if r.ID == rest2.ID { found2 = true }
	}
